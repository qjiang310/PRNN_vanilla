...
def getLibCXXPaths():
    ...
    lib_path = '/usr/lib/x86_64-linux-gnu/libc++.so.1'
    # else:
...
def updateEnvironment(env):
    ...
    for key, value in originalEnvironment.items(): #FIXME iteritems()
        env[key] = value
...
def BuildEnvironment():
    ...
    if sys.platform == 'linux2':
        ...
        env.AppendUnique(EXTRA_LIBS = ['-ldl']) #FIXME
        env.AppendUnique(EXTRA_LIBS = ['-lineinfo']) 

    # we need libdl on max and linux
    if os.name != 'nt':
        env.AppendUnique(EXTRA_LIBS = ['-ldl'])
        env.AppendUnique(EXTRA_LIBS = ['-lrt']) #FIXME
        # env.AppendUnique(EXTRA_LIBS = ['-lineinfo'])
